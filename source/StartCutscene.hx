...
class StartCutscene extends UIStateExt {
	...
	override public function create() {
		...
		for (i in [frameSprite, pressAnyKey]) add(i);
	...
	override function update(elapsed) {
		...
		if (FlxG.keys.anyJustPressed(FlxG.sound.volumeDownKeys.concat(FlxG.sound.volumeUpKeys).concat(FlxG.sound.muteKeys))) {} // is there no other way >:(
		else if (FlxG.keys.justPressed.ANY) advance();
	}

	function advance() {
		if (curFrame >= frames.length) return;
		...
		if (curFrame == frames.length) {
			FlxG.camera.fade(0xFF4F2632, 1, false, function() {
				new FlxTimer().start(0.5, function(t) {
					switchState(new MainMenu()); // the true advance
				});
			});
			FlxG.sound.music.fadeOut(1, 0, function(t) {
				FlxG.sound.music.stop();
			});
		} else if (curFrame < frames.length) frameSprite.loadGraphic("assets/images/cutscene/" + frames[curFrame] + ".png");
	}
